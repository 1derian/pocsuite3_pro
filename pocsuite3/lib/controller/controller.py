...
def show_task_result():
    ...
    if not kb.results:
        ...
        # 所有线程poc扫描结束后,默认poc的返回值都追加到results列表中
        # 需求,把所有结果中存在漏洞的根据当前的任务追加到一个文本中去,方便后期单独利用
        # 提交了文件保存的参数 , 目前没有解决的问题,保存路径不能存在多级不存在的目录
        if conf.save_file:
            # 转换成绝对路径
            full_path = os.path.abspath(conf.save_file)
            # 拿到上一层目录
            ...
            if not is_exist:
                # 不能创建多层目录
                os.mkdir(dir_path)
            with open(f"{full_path}", mode='w', encoding="utf-8") as f:
                for res in kb.results:
